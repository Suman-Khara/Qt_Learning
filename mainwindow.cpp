...
void MainWindow::on_Flood_Fill_clicked()
{
    ...
    while (!stack.isEmpty()) {
        ...
        if (!visited.contains({pixelX, pixelY})) {
            ...
            colorPoint(pixelX, pixelY, 255, 0, 0, gridOffset);
            visited.insert({pixelX, pixelY});
            Delay;

            stack.enqueue(QPoint(x + 1, y));
            stack.enqueue(QPoint(x - 1, y));
            stack.enqueue(QPoint(x, y + 1));
            stack.enqueue(QPoint(x, y - 1));
            if(c==8){
                stack.enqueue(QPoint(x + 1, y+1));
                stack.enqueue(QPoint(x - 1, y-1));
                stack.enqueue(QPoint(x-1, y + 1));
                stack.enqueue(QPoint(x+1, y - 1));
            }
...
void MainWindow::on_Boundary_Fill_clicked()
{
    ...
    int c = ui->Connected->value();
    if (c != 4 && c != 8) {
        ui->Boundary_Fill_Time->setText("Wrong connectedness");
        return;
    }
    if (clickedPoints.empty()) {
    ...
    while (!stack.isEmpty()) {
        ...
        // Check if the point is within bounds, not visited, and not a boundary
        if (!visited.contains({pixelX, pixelY}) && !allPolygonPoints.contains(QPoint(x, y)) && currentColor != QColor(r, g, b)) {
            ...
            // Add neighboring points to stack based on connectedness
